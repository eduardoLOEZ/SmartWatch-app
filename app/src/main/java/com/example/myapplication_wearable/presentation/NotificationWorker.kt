...
class NotificationWorker(context: Context, params: WorkerParameters) : Worker(context, params) {
    ...
    override fun doWork(): Result {
        sendNotification()
        scheduleNextNotification() // Programar el próximo trabajo
    ...
    private fun sendNotification() {
        if (ContextCompat.checkSelfPermission(applicationContext, Manifest.permission.POST_NOTIFICATIONS)
            ...
            val notificationId = System.currentTimeMillis().toInt()  // Identificador único
            ...
            val builder = NotificationCompat.Builder(applicationContext, "CHALLENGE_CHANNEL")
                ...
                .setGroup("CHALLENGE_GROUP")  // Grupo de notificaciones

            with(NotificationManagerCompat.from(applicationContext)) {
                notify(notificationId, builder.build())  // Notificación individual
            }
        } else {
